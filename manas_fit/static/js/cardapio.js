...
function sendWhatsapp() {
  ...
  const pay    = chosenPayment === 'pix' ? 'PIX' : (needsChange ? `Dinheiro (troco de R$ ${(parseFloat(document.getElementById('cashAmount')?.value || 0)).toFixed(2).replace('.',',')})` : 'Dinheiro (sem troco)');
...
function addToCart(name, price, consult = false) {
  if (!IS_LOGGED) { openCart(); return; }
  if (consult) {
    showToast('💬 Entre em contato pelo WhatsApp para consultar!');
    return;
  }
...
function selectMulti(btn, group, max) {
  // Decide which state object to use
  ...
  if (btn.classList.contains('selected')) {
    ...
    if ((stateObj[group]?.length || 0) >= max) {
      showToast(`⚠️ Máximo de ${max} opção(ões) para este campo!`);
      return;
    }
    btn.classList.add('selected');
...
function updateBuilderSummary() {
  ...
  list.innerHTML = items.length ? items.join('') : '<li style="color:var(--muted); font-style:italic;">Nenhum ingrediente selecionado ainda...</li>';
}
...
function selectMarmitaSize(btn, size) {
  ...
  marmitaSize  = size;
  marmitaCombo = null;
  document.getElementById('combos450').style.display = size === '450g' ? 'block' : 'none';
  document.getElementById('combos750').style.display = size === '750g' ? 'block' : 'none';
...
function updateMarmitaPrice() {
  ...
  if (!marmitaCombo) {
    ...
    labelEl.textContent = 'Selecione um combo acima';
    return;
  }
...
function addMarmitaToCart() {
  ...
  const name = `Marmita ${marmitaSize} (${marmitaCombo.qty}x)`;
  cart.push({ name, price: marmitaCombo.price, qty: 1, details });
  updateBadge();
...
function updateMbSummary() {
  ...
  list.innerHTML = items.length ? items.join('') : '<li style="color:var(--muted); font-style:italic;">Nenhuma opção selecionada ainda...</li>';
}
...
function selectMassaSize(btn, size) {
  ...
  mbSize  = size;
  mbCombo = null;
  document.getElementById('combosMB450').style.display = size === '450g' ? 'block' : 'none';
  document.getElementById('combosMB750').style.display = size === '750g' ? 'block' : 'none';
  document.querySelectorAll('#combosMB450 .combo-card, #combosMB750 .combo-card').forEach(c => c.classList.remove('selected'));
  updateMbSummary();
  updateMbPrice();
}
...
function addMassaToCart() {
  ...
  const name    = `Massa ao Molho Branco ${mbSize} (${mbCombo.qty}x)`;
  cart.push({ name, price: mbCombo.price, qty: 1, details });
  updateBadge();
