...
def validate_password(pw: str) -> bool:
    ...
    if not re.search(r'\d', pw):
        return False
    if re.search(r'[\s\u00C0-\u024F]', pw):  # no spaces or accents
        return False
    return True
...
def register():
    ...
    if not validate_password(password):
        return jsonify(ok=False, msg='A senha deve ter mais de 6 caracteres, com letras maiúsculas, minúsculas e números, sem espaços ou caracteres especiais.')

    # Uniqueness
    if User.query.filter_by(cpf=re.sub(r'\D','',cpf)).first():
        return jsonify(ok=False, msg='Este CPF já está cadastrado.')
    if User.query.filter_by(phone=re.sub(r'\D','',phone)).first():
